...
def epsilon_greedy(state_vector, epsilon):
    ...
    if np.random.random() < epsilon:
        ...
        next_action = torch.argmax(q_actions)
        next_object = torch.argmax(q_objects)
...
def deep_q_learning(current_state_vector, action_index, object_index, reward,
    ...
    q_value_action_cur = q_values_action_cur[action_index]
    q_value_object_cur = q_values_object_cur[object_index]
    q_value_cur = 1 / 2 * (q_value_action_cur + q_value_object_cur)
...
def run_episode(for_training):
    ...
    while not terminal:
        ...
        next_action, next_object = epsilon_greedy(current_state_vector, epsilon)
        ...
        if for_training:
            ...
            deep_q_learning(current_state_vector, next_action, next_object, reward, next_state_vector, terminal)
