...
def epsilon_greedy(state_vector, theta, epsilon):
    ...
    if np.random.random() < epsilon:
        ...
        q_value = np.matmul(theta, state_vector)
        q_max = np.argmax(q_value)
        next_action, next_object = index2tuple(q_max)
...
def linear_q_learning(theta, current_state_vector, action_index, object_index,
    ...
    Returns:
        None
    """
    q_value = np.matmul(theta, current_state_vector)
    idx_current_action = tuple2index(action_index, object_index)

    if terminal:
        loss_neg_derivative = (reward - q_value[idx_current_action]) * current_state_vector
    else:
        q_value_next = np.matmul(theta, next_state_vector)
        q_max = np.argmax(q_value_next)
        loss_neg_derivative = (reward + GAMMA * q_value_next[q_max] - q_value[idx_current_action]) * current_state_vector

    theta[idx_current_action] += ALPHA * loss_neg_derivative
...
def run_episode(for_training):
    ...
    while not terminal:
        ...
        next_action, next_object = epsilon_greedy(current_state_vector, theta, epsilon)
        ...
        if for_training:
            ...
            linear_q_learning(theta, current_state_vector, next_action, next_object, reward, next_state_vector, terminal)
